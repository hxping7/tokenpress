...
if %errorlevel% equ 0 (
    powershell -NoProfile -ExecutionPolicy Bypass -Command ^
        "$src = [System.IO.File]::OpenRead('%PROJECT_DIR:\=\\%\\tokenpress-backend.tar'); " ^
        "$dst = [System.IO.File]::Create('%PROJECT_DIR:\=\\%\\tokenpress-backend.tar.gz'); " ^
        "$gs = New-Object System.IO.Compression.GzipStream $dst ([System.IO.Compression.CompressionMode]::Compress); " ^
        "$src.CopyTo($gs); $gs.Close(); $dst.Close(); $src.Close()"
    if errorlevel 1 (
...
if %errorlevel% equ 0 (
    powershell -NoProfile -ExecutionPolicy Bypass -Command ^
        "$src = [System.IO.File]::OpenRead('%PROJECT_DIR:\=\\%\\tokenpress-frontend.tar'); " ^
        "$dst = [System.IO.File]::Create('%PROJECT_DIR:\=\\%\\tokenpress-frontend.tar.gz'); " ^
        "$gs = New-Object System.IO.Compression.GzipStream $dst ([System.IO.Compression.CompressionMode]::Compress); " ^
        "$src.CopyTo($gs); $gs.Close(); $dst.Close(); $src.Close()"
    if errorlevel 1 (
        echo [BUILD] [WARN] PowerShell gzip failed, using uncompressed .tar
