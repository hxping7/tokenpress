 ...
 * 鉴权预检。
 *
 * 为什么必须做：**鉴权失败 ≠ 站点为空**。token 失效时 `/ai/articles` 读不到东西，
 * 若不区分，自检会报「文章数量 0 —— 尚未发布内容」把问题掩盖过去（还会连带跳过
 * 文章页可达与媒体引用两项检查，看起来「只是没内容」）。
 *
 * 用 `/ai/articles` 探测，靠错误文案/状态码区分：
 * - 401 / `API token not found|revoked|expired` → token 不可用（致命）
 * - 403 / `Missing required permission: x`      → token 有效，缺该权限（检查会不完整）
 */
async function preflight(apiBase, token) {
  ...
  if (res.status === 401 || /API token (not found|has been revoked|has expired)/i.test(err)) {
    return {
      fatal: `Token 不可用 — ${err || 'HTTP ' + res.status}`,
      hint: 'token 形如 t00_sk_ + 48 位十六进制（共 55 字符）：① 确认 .token00.conf 里的值完整（换行/复制粘贴常被截断，可打印长度核对）② 到后台「API Token」确认未被撤销或过期',
    }
  }
  if (res.status === 403 || /Missing required permission/i.test(err)) {
    return { missing: err.replace(/^.*permission:\s*/i, '').trim() }
  }
  return {}
}
...
async function main() {
  ...
  // ⓪ 鉴权预检：区分「token 不可用」与「站点为空」——不区分就会把读不到误报成没有内容
  const pre = await preflight(apiBase, token)
  if (pre.fatal) {
    bad('Token 鉴权', pre.fatal)
    if (pre.hint) console.log('\n  ' + pre.hint)
    console.log('\n  结果：自检中止（鉴权失败，后续检查无意义）  exit=1')
    process.exit(1)
  }
  if (pre.missing) warn('Token 鉴权', `有效，但缺少 ${pre.missing} —— 文章列表相关检查不完整`)
  else ok('Token 鉴权', '有效')
  ...
  const listFailed = list.success === false
  ...
  if (listFailed) bad('文章列表可读', `请求失败：${list.error || '未知错误'}（文章相关检查已跳过）`)
  else if (!articles.length) warn('文章数量', '0 —— 尚未发布内容')
  ...
  if (!mediaList.length) {
    if (!detailTargets.length) warn('媒体引用', listFailed ? '已跳过（文章列表读取失败，见上）' : '没有文章可检查')
    else warn('媒体引用', detailOk ? '没有可检查的媒体引用' : '未能读取文章详情的正文/封面')
  } else {
