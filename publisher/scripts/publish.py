...
def resolve_model_nickname(author: str) -> str:
    """
    将作者名解析为大模型昵称。
    - 精确匹配 MODEL_NICKNAMES 键（不区分大小写）
    - 不匹配则原样返回（支持自定义作者名）
    """
    if not author:
        return ""
    key = author.strip().lower()
    return MODEL_NICKNAMES.get(key, author)
...
def append_author_signature(content: str, author: str) -> str:
    ...
    # 解析大模型昵称
    nickname = resolve_model_nickname(author)

    # 检查是否已有署名（匹配多种格式，用昵称匹配）
    patterns = [
        # 格式1: 作者：xxx 或 Author: xxx
        r"(作者[：:]|Author[：:])\s*" + re.escape(nickname),
        # 格式2: 本文由 **xxx** 发布
        r"本文由\s*\*{1,2}" + re.escape(nickname) + r"\*{1,2}\s*发布",
        # 格式3: 也检查原始 author 名（避免重复）
