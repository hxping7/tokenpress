...
router.post('/', apiTokenOrAdmin('site:write'), async (req: AuthRequest, res) => {
  try {
...
router.put('/:id', apiTokenOrAdmin('site:write'), async (req: AuthRequest, res) => {
  try {
...
router.patch('/:id/reset-password', apiTokenOrAdmin('site:write'), async (req: AuthRequest, res) => {
  try {
    const userId = getParamAsInt(req.params.id)
    if (!userId) {
      return res.status(400).json({ success: false, error: 'Invalid ID' })
...
router.delete('/:id', apiTokenOrAdmin('site:write'), async (req: AuthRequest, res) => {
  try {
    const userId = getParamAsInt(req.params.id)
    if (!userId) {
      return res.status(400).json({ success: false, error: 'Invalid ID' })
